...
export function PlayerProfile() {
  ...
  const savedFlag = useRef<string | null>(null);
  ...
  async function editName() {
    setMessage(""); setBusy(true); setLoaded(false);
    try {
      ...
      if (current) {
        const { data, error } = await db.from("profiles").select("display_name, flag_code").eq("id", current.id).maybeSingle();
        if (error) throw error;
        savedName.current = data?.display_name ?? "";
        setName(savedName.current);
        savedFlag.current = data?.flag_code ?? null;
        setFlag(savedFlag.current);
      } else {
        savedName.current = ""; savedFlag.current = null;
        setName(""); setFlag(null);
      }
      setLoaded(true);
  ...
  async function closeProfile() {
    if (busy) return;
    if (!loaded) { dialog.current?.close(); return; }
  ...
  async function persistName(): Promise<boolean> {
    ...
    if (value === savedName.current && flag === savedFlag.current) return true;
    if ((value || savedName.current) && ([...value].length < 2 || [...value].length > 24 || /[\p{Cc}\p{Cf}]/u.test(value))) {
    ...
    try {
      ...
      const displayName = value || `Pilot-${current.id.replaceAll("-", "").slice(0, 18)}`;
      const { error } = await db.from("profiles").upsert({ id: current.id, display_name: displayName, flag_code: flag }, { onConflict: "id" });
      if (error) throw new Error(error.code === "23505" ? "That display name is taken. Try another." : error.message);
      savedName.current = displayName; savedFlag.current = flag;
      setName(displayName); setUser(current); setMessage("Profile saved.");
      return true;
  ...
  async function signIn() {
    ...
    try {
      ...
      if (!settings.external?.discord) throw new Error("Discord sign-in is not available yet.");
  ...
  return <>
    ...
    <dialog ref={dialog} className="mobile-menu-dialog player-profile-dialog" aria-labelledby="profile-title" onCancel={event => { event.preventDefault(); void closeProfile(); }}>
      ...
      <button className="profile-option discord-sign-in" disabled={!!discord || busy || !loaded} onClick={() => void signIn()}>
      ...
      <form onSubmit={event => { event.preventDefault(); void saveName(); }}>
        <input className="profile-option" id="player-display-name" aria-label="Display name" placeholder="Enter your name here" value={name} onChange={event => setName(event.target.value)} autoComplete="nickname" disabled={busy || !loaded} />
        <FlagPicker value={flag} onChange={setFlag} disabled={busy || !loaded} />
        <button className="profile-option profile-save" type="submit" disabled={busy || !loaded}>{busy ? "Please wait…" : "Save profile"}</button>
      </form>
      {!loaded && !busy && <button className="profile-option" onClick={() => void editName()}>Retry loading profile</button>}
